...
本文档概述 codex-switch 的目标、核心能力与架构设计，帮助贡献者快速理解代码结构与主要流程。

---

## 1. 项目目标

codex-switch 希望解决以下痛点：
...
5. **自动化发布**  
   提供 GitHub Actions workflow 与本地 `scripts/release.sh`，可校验编译并自动创建/推送标签，由 Actions 统一生成发布制品。
...
- 本地脚本 `scripts/release.sh <version> [notes]` 用于校验编译并推送标签，随后由 GitHub Actions 生成 Release。
